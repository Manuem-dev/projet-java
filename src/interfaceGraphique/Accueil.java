...
/**
 * Fenêtre de connexion — reçoit tous les services 
 */
public class Accueil extends JFrame {
	...
	public Accueil(GestionUtilisateur gestionUtilisateur) {
		this(gestionUtilisateur, new GestionProduit(), new GestionEmploye(), new GestionClient(), new GestionRayon(),
				new GestionCaisseVente(), new GestionFacture());
	}

	// Constructeur complet — reçoit les services 
	public Accueil(GestionUtilisateur pGestionUtilisateur, GestionProduit pGestionProduit, GestionEmploye pGestionEmploye,
			GestionClient pGestionClient, GestionRayon pGestionRayon, GestionCaisseVente pGestionCaisseVente,
			GestionFacture pGestionFacture) {
		gestionUtilisateur = pGestionUtilisateur;
		gestionProduit = pGestionProduit;
		gestionEmploye = pGestionEmploye;
		gestionClient = pGestionClient;
		gestionRayon = pGestionRayon;
		gestionCaisseVente = pGestionCaisseVente;
		gestionFacture = pGestionFacture;
	...
	private void buildUI() {
		...
		form.setBorder(BorderFactory.createCompoundBorder(new LineBorder(new Color(0xDDDDDD), 1, true),
		...
		btnConnecter.addMouseListener(new MouseAdapter() {
			public void mouseEntered(MouseEvent e) {
				btnConnecter.setBackground(ACCENT);
			}
	...
	private void tentativeConnexion() {
		...
		Utilisateur utilisateur = gestionUtilisateur.connecter(login, mdp);
		if (utilisateur != null) {
			lblMessage.setForeground(new Color(0x2E7D32));
			lblMessage.setText("Bienvenue, " + utilisateur.getEmploye().getPrenom() + " !");
			SwingUtilities.invokeLater(() -> {
				new Dashboard(gestionUtilisateur, gestionProduit, gestionEmploye, gestionClient, gestionRayon,
	...
	private void styleTextField(JTextField tf) {
		tf.setFont(FONT_LBL);
		tf.setBorder(BorderFactory.createCompoundBorder(new LineBorder(new Color(0xBDBDBD), 1, true),
