...
public class FenetreCaisse extends JPanel {
    ...
    public FenetreCaisse(GestionCaisseVente gestionCaisseVente,
    ...
    private void buildUI() {
        ...
        table.setFont(FONT_LBL); table.setRowHeight(26);
        ...
        tfRef = new JTextField(8); tfRef.setFont(FONT_LBL);
        tfQte = new JTextField("1", 4); tfQte.setFont(FONT_LBL);
        ...
        JTextField tfMontant = new JTextField(10); tfMontant.setFont(FONT_LBL);
        JButton btnChoisirMode = blueBtn("Choisir mode paiement…");
        final String[] modeChoisi = {Paiement.MODE_ESPECES};
        JLabel lblMode = new JLabel("Mode : " + modeChoisi[0]);
        lblMode.setFont(FONT_LBL); lblMode.setForeground(PRIMARY);
        btnChoisirMode.addActionListener(e -> {
            String[] modes = {Paiement.MODE_ESPECES, Paiement.MODE_CARTE, Paiement.MODE_CHEQUE};
            ...
            if (JOptionPane.showConfirmDialog(this, p, "Mode de paiement", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE) == JOptionPane.OK_OPTION
                    && lst.getSelectedIndex() >= 0) {
        ...
        btnPayer.setBackground(GREEN); btnPayer.setForeground(Color.WHITE);
        btnPayer.setContentAreaFilled(false);
        btnPayer.setFocusPainted(false); btnPayer.setBorderPainted(false);
        btnPayer.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        btnPayer.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) { btnPayer.setBackground(new Color(0x1B5E20)); }
            public void mouseExited(MouseEvent e)  { btnPayer.setBackground(GREEN); }
        });
    ...
    private void choisirCaissier() {
        ...
        for (Employe e : employes) {
            if (e instanceof Caissier) caissiers.add((Caissier) e);
        }
        ...
        for (int i = 0; i < caissiers.size(); i++)
            noms[i] = caissiers.get(i).getPrenom() + " " + caissiers.get(i).getNom()
                    + " (mat. " + caissiers.get(i).getMatricule() + ")";
        ...
        if (JOptionPane.showConfirmDialog(this, p, "Choisir un caissier", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE) == JOptionPane.OK_OPTION
                && lst.getSelectedIndex() >= 0) {
    ...
    private void choisirClient() {
        ...
        if (JOptionPane.showConfirmDialog(this, p, "Choisir un client", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE) == JOptionPane.OK_OPTION
                && lst.getSelectedIndex() >= 0) {
    ...
    private void choisirProduit() {
        ...
        for (int i = 0; i < produits.size(); i++)
            noms[i] = "[" + produits.get(i).getReference() + "] " + produits.get(i).getDesignation()
                    + " — " + String.format("%.2f €", produits.get(i).getPrixVente())
                    + " (stock : " + produits.get(i).getQuantiteStock() + ")";
        ...
        if (JOptionPane.showConfirmDialog(this, p, "Choisir un produit", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE) == JOptionPane.OK_OPTION
                && lst.getSelectedIndex() >= 0) {
    ...
    private void ajouterProduitPanier() {
        ...
        try {
            ...
            if (p == null) { JOptionPane.showMessageDialog(this, "Produit réf. " + ref + " introuvable.", "Erreur", JOptionPane.ERROR_MESSAGE); return; }
    ...
    private void retirerProduitPanier() {
        if (venteEnCours == null) return;
        int row = table.getSelectedRow();
        if (row < 0) return;
        int ref = (int) tableModel.getValueAt(row, 0);
        Produit p = gestionProduit.rechercherProduit(ref);
        if (p != null) { gestionCaisseVente.retirerProduitVente(venteEnCours, p); refreshPanier(); }
    }

    private void finaliserVente(String mode, JTextField tfMontant) {
        if (venteEnCours == null) { JOptionPane.showMessageDialog(this, "Aucune vente en cours.", "Info", JOptionPane.WARNING_MESSAGE); return; }
        try {
            ...
            if (f != null) {
                JOptionPane.showMessageDialog(this, f.getContenuFacture(), "Facture n°" + f.getNumeroFacture(), JOptionPane.INFORMATION_MESSAGE);
                venteEnCours = null; clientEnCours = null; caissierActif = null;
                tableModel.setRowCount(0);
                lblTotal.setText("Total : 0,00 €");
                lblCaissier.setText("— aucun —"); lblCaissier.setForeground(new Color(0x757575));
                lblClientInfo.setText("— aucun —"); lblClientInfo.setForeground(new Color(0x757575));
            }
        } catch (NumberFormatException ex) {
    ...
    private void refreshPanier() {
        ...
        for (java.util.Map.Entry<Produit, Integer> e : occ.entrySet()) {
            Produit p = e.getKey(); int qte = e.getValue();
            tableModel.addRow(new Object[]{
                    p.getReference(), p.getDesignation(), qte,
                    String.format("%.2f", p.getPrixVente()),
                    String.format("%.2f", p.getPrixVente() * qte)
            });
        }
    ...
    private JPanel section(String titre) {
        ...
        p.setBorder(BorderFactory.createTitledBorder(
                new LineBorder(new Color(0xBDBDBD), 1, true), " " + titre + " ",
    ...
    private JPanel rowOf(String lbl, JComponent comp) {
        ...
        JLabel l = new JLabel(lbl); l.setFont(FONT_LBL); l.setForeground(TEXT_FG);
