...
public class FenetreClient extends JPanel {
    ...
    private void buildUI() {
        ...
        tfRecherche = new JTextField(18); tfRecherche.setFont(FONT_LBL);
        ...
        btnTout.addActionListener(e -> { tfRecherche.setText(""); refreshTable(gestionClient.getListeClients()); });
        barreRecherche.add(new JLabel("Nom : ")); barreRecherche.add(tfRecherche);
        barreRecherche.add(btnRech); barreRecherche.add(btnTout);
        ...
        formPanel.setBorder(BorderFactory.createTitledBorder(
                new LineBorder(new Color(0xBDBDBD), 1, true), " Ajouter / Supprimer un client ",
        ...
        g.gridx=0; g.gridy=0; formPanel.add(new JLabel("Nom :"), g);
        g.gridx=1; tfNom = new JTextField(16); formPanel.add(tfNom, g);
        g.gridx=2; formPanel.add(new JLabel("Téléphone :"), g);
        g.gridx=3; tfTelephone = new JTextField(12); formPanel.add(tfTelephone, g);
        ...
        btnRow.add(btnAjouter); btnRow.add(btnHistorique); btnRow.add(btnSupprimer);
    ...
    private void refreshTable(ArrayList<Client> liste) {
        ...
        for (Client c : liste) {
            tableModel.addRow(new Object[]{
                    c.getNumeroClient(), c.getNom(), c.getTelephone(),
                    c.getPointsFidelite(), c.getHistoriqueAchats().size()
            });
        }
    }
    ...
    private void ajouterClient() {
        ...
        refreshTable(gestionClient.getListeClients());
        tfNom.setText(""); tfTelephone.setText("");
        JOptionPane.showMessageDialog(this, "Client ajouté !", "Succès", JOptionPane.INFORMATION_MESSAGE);
    }

    private void supprimerClient() {
        int row = table.getSelectedRow();
        if (row < 0) { JOptionPane.showMessageDialog(this, "Sélectionnez un client.", "Info", JOptionPane.WARNING_MESSAGE); return; }
        int num = (int) tableModel.getValueAt(row, 0);
        int rep = JOptionPane.showConfirmDialog(this, "Supprimer ce client ?", "Confirmation", JOptionPane.YES_NO_OPTION);
    ...
    private void voirHistorique() {
        ...
        if (c.getHistoriqueAchats().isEmpty()) {
            ...
            for (var v : c.getHistoriqueAchats()) {
                sb.append("• Vente n°").append(v.getNumeroVente())
                  .append(" | ").append(v.getDate())
                  .append(" | ").append(String.format("%.2f €", v.calculerTotal())).append("\n");
            }
        }
        JOptionPane.showMessageDialog(this, sb.toString(), "Historique de " + c.getNom(), JOptionPane.INFORMATION_MESSAGE);
    }
