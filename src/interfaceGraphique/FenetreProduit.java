...
/**
 * Panneau de gestion des produits. Sélection du type via pop-up de choix,
...
public class FenetreProduit extends JPanel {
	...
	private void buildUI() {
		...
		formPanel.setBorder(BorderFactory.createTitledBorder(new LineBorder(new Color(0xBDBDBD), 1, true),
				" Ajouter / Modifier un produit ", TitledBorder.LEFT, TitledBorder.TOP, FONT_LBL.deriveFont(Font.BOLD),
				PRIMARY));
		...
		g.gridx = 0;
		g.gridy = 0;
		formPanel.add(lbl("Type de produit :"), g);
		g.gridx = 1;
		...
		g.gridx = 3;
		formPanel.add(lbl("Réf. :"), g);
		g.gridx = 4;
		tfRef = new JTextField(6);
		formPanel.add(tfRef, g);

		// Ligne 1
		g.gridx = 0;
		g.gridy = 1;
		formPanel.add(lbl("Désignation :"), g);
		g.gridx = 1;
		g.gridwidth = 4;
		tfDesignation = new JTextField(26);
		formPanel.add(tfDesignation, g);
		g.gridwidth = 1;

		// Ligne 2
		g.gridx = 0;
		g.gridy = 2;
		formPanel.add(lbl("Quantité :"), g);
		g.gridx = 1;
		tfQte = new JTextField(7);
		formPanel.add(tfQte, g);
		g.gridx = 2;
		formPanel.add(lbl("Prix achat (€) :"), g);
		g.gridx = 3;
		tfPrixAchat = new JTextField(7);
		formPanel.add(tfPrixAchat, g);
		g.gridx = 4;
	...
	private void choisirTypeProduit() {
		...
		int res = JOptionPane.showConfirmDialog(this, panel, "Type de produit", JOptionPane.OK_CANCEL_OPTION,
	...
	private void refreshTable(ArrayList<Produit> liste) {
		...
		for (Produit p : liste) {
			...
			tableModel.addRow(new Object[] { p.getReference(), p.getDesignation(), type, p.getQuantiteStock(),
	...
	private void ajouterProduit() {
		if (selectedType == null) {
			JOptionPane.showMessageDialog(this, "Veuillez d'abord choisir le type de produit.", "Type manquant",
		...
		try {
			...
			case "Artisanal":
				...
				pa_form.add(new JLabel("Catégorie :"));
				pa_form.add(tfCateg);
				pa_form.add(new JLabel("Artisan :"));
				pa_form.add(tfArtisan);
				pa_form.add(new JLabel("Origine :"));
				pa_form.add(tfOrigine);
				if (JOptionPane.showConfirmDialog(this, pa_form, "Détails Artisanal",
						JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION)
					gestionProduit.ajoutProduitArtisanal(ref, qte, pa, pv, des, tfCateg.getText(), tfArtisan.getText(),
							tfOrigine.getText());
				break;
			case "Électronique":
				...
				pe_form.add(new JLabel("Marque :"));
				pe_form.add(tfMarque);
				pe_form.add(new JLabel("Garantie (mois) :"));
				pe_form.add(tfGarantie);
				if (JOptionPane.showConfirmDialog(this, pe_form, "Détails Électronique",
						JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
					int gar = tfGarantie.getText().trim().isEmpty() ? 12
							: Integer.parseInt(tfGarantie.getText().trim());
			...
			case "Frais":
				...
				pf_form.add(new JLabel("Température (°C) :"));
				pf_form.add(tfTemp);
				pf_form.add(new JLabel("Date péremption (YYYY-MM-DD) :"));
				pf_form.add(tfDate);
				if (JOptionPane.showConfirmDialog(this, pf_form, "Détails Frais",
						JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
	...
	private void supprimerProduit() {
		...
		if (JOptionPane.showConfirmDialog(this, "Supprimer ce produit ?", "Confirmation",
	...
	private void chargerLigne() {
		...
		selectedType = type;
		lblType.setText(
				type.equals("Frais") ? "❄️ Frais" : type.equals("Électronique") ? "⚡ Électronique" : "📦 Artisanal");
		lblType.setForeground(PRIMARY);
		lblType.setFont(FONT_LBL.deriveFont(Font.BOLD));
	...
	private JButton grayBtn(String t) {
		...
		b.setFont(FONT_LBL);
		b.setBackground(new Color(0x757575));
		b.setForeground(Color.WHITE);
		b.setContentAreaFilled(false);
		b.setFocusPainted(false);
		b.setBorderPainted(false);
		...
		b.addMouseListener(new MouseAdapter() {
			public void mouseEntered(MouseEvent e) {
				b.setBackground(new Color(0x616161));
			}
