...
public class FenetreStock extends JPanel {
    ...
    public FenetreStock(GestionProduit gestionProduit, GestionRayon gestionRayon) {
    ...
    private void buildUI() {
        ...
        panelRayons.setBorder(BorderFactory.createTitledBorder(
                new LineBorder(new Color(0xBDBDBD), 1), " Rayons ",
                TitledBorder.LEFT, TitledBorder.TOP, FONT_LBL.deriveFont(Font.BOLD), PRIMARY));
        String[] colsR = {"Code", "Nom rayon", "Responsable", "Produits"};
        tableModelRayons = new DefaultTableModel(colsR, 0) { public boolean isCellEditable(int r, int c) { return false; } };
        tableRayons = new JTable(tableModelRayons);
        tableRayons.setFont(FONT_LBL); tableRayons.setRowHeight(24);
        ...
        g.gridx=0; g.gridy=0; formRayon.add(new JLabel("Nom rayon :"), g);
        g.gridx=1; formRayon.add(tfNomRayon, g);
        g.gridx=0; g.gridy=1; formRayon.add(new JLabel("Matricule chef :"), g);
        g.gridx=1; formRayon.add(tfMatResp, g);
        ...
        btnAddRayon.addActionListener(e -> {
            ...
            refreshTables();
            tfNomRayon.setText(""); tfMatResp.setText("");
        });
        ...
        panelProduits.setBorder(BorderFactory.createTitledBorder(
                new LineBorder(new Color(0xBDBDBD), 1), " Tous les produits & état du stock ",
                TitledBorder.LEFT, TitledBorder.TOP, FONT_LBL.deriveFont(Font.BOLD), PRIMARY));
        String[] colsP = {"Réf.", "Désignation", "Type", "Stock", "Alerte"};
        tableModelProduits = new DefaultTableModel(colsP, 0) { public boolean isCellEditable(int r, int c) { return false; } };
        tableProduits = new JTable(tableModelProduits);
        tableProduits.setFont(FONT_LBL); tableProduits.setRowHeight(24);
        ...
        tableProduits.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            public Component getTableCellRendererComponent(JTable t, Object v, boolean sel, boolean foc, int r, int c) {
                ...
                if ("⚠️ Stock faible".equals(alerte)) setBackground(new Color(0xFFF9C4));
                else if ("🔴 Rupture".equals(alerte)) setBackground(new Color(0xFFCDD2));
                else setBackground(Color.WHITE);
                if (sel) setBackground(new Color(0xBBDEFB));
                return this;
            }
        });
        ...
        btnSortieStock.setFont(FONT_LBL); btnSortieStock.setBackground(ORANGE);
        btnSortieStock.setForeground(Color.WHITE); btnSortieStock.setContentAreaFilled(false); btnSortieStock.setFocusPainted(false); btnSortieStock.setBorderPainted(false);
        ...
        btnStock.add(btnAjoutStock); btnStock.add(btnSortieStock); btnStock.add(btnRefresh);
    ...
    private void refreshTables() {
        ...
        for (Rayon r : gestionRayon.getListeRayons()) {
            tableModelRayons.addRow(new Object[]{
                    r.getCodeRayon(), r.getNomRayon(),
                    r.getResponsable().getPrenom() + " " + r.getResponsable().getNom(),
        ...
        for (Produit p : gestionProduit.getListeDesProduits()) {
            ...
            tableModelProduits.addRow(new Object[]{
                    p.getReference(), p.getDesignation(), type, p.getQuantiteStock(), alerte
            });
        }
    }
    ...
    private void mouvementStock(boolean entree) {
        ...
        String input = JOptionPane.showInputDialog(this, msg, entree ? "Entrée stock" : "Sortie stock", JOptionPane.QUESTION_MESSAGE);
        if (input == null || input.trim().isEmpty()) return;
        try {
            int qte = Integer.parseInt(input.trim());
            if (entree) p.ajouterStock(qte);
            else p.retirerStock(qte);
            refreshTables();
