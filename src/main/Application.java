...
/**
 * Point d'entrée de l'application Gestion Supermarché.
 * Initialise des objets de test et lance l'interface graphique.
 */
public class Application {
    ...
    public static void main(String[] args) {
        ...
        // ─── EXERCICE 1 : Produits ────────────────────────────────────────
        System.out.println("\n=== EXERCICE 1 : PRODUITS ===");
        gestionProduit.ajoutProduitArtisanal(101, 50, 1.2, 2.5, "Baguette",
                "Boulangerie", "Boulangerie Martin", "France");
        gestionProduit.ajoutProduitArtisanal(102, 30, 8.0, 15.0, "Filet de saumon",
                "Poissonnerie", "Poissonnerie Duval", "Norvège");
        gestionProduit.ajoutProduitArtisanal(103, 20, 5.0, 12.0, "Côte de bœuf",
                "Boucherie", "Boucherie Pascal", "France");
        gestionProduit.ajoutProduitElectronique(201, 15, 180.0, 299.99, "Télévision 40\"",
                "Samsung", 24);
        gestionProduit.ajoutProduitElectronique(202, 8, 50.0, 89.99, "Casque Bluetooth",
                "Sony", 12);
        gestionProduit.ajoutProduitFrais(301, 80, 0.8, 1.99, "Yaourt nature",
                4.0, LocalDate.now().plusDays(15));
        gestionProduit.ajoutProduitFrais(302, 5, 1.5, 3.99, "Lait entier (1L)",
                6.0, LocalDate.now().plusDays(7));
        // Produit périmé pour les tests
        // gestionProduit.ajoutProduitFrais(303, 3, 0.5, 1.0, "Yaourt périmé", 4.0, LocalDate.now().minusDays(2));
        ...
        System.out.println("Alertes stock (seuil 10) : " + stock.verifierStockMinimum(gestionProduit.getListeDesProduits(), 10).size());
        ...
        Magasinier magasinier = new Magasinier(30,"Renard",    "Kevin",   2400, "Entrepôt A");
        ...
        Facture facture1 = gestionCaisseVente.finaliserVente(vente1, Paiement.MODE_ESPECES, 20.0);
        if (facture1 != null) gestionFacture.enregistrerFacture(facture1);
        ...
        Facture facture2 = gestionCaisseVente.finaliserVente(vente2, Paiement.MODE_CARTE, 89.99);
        if (facture2 != null) gestionFacture.enregistrerFacture(facture2);
        ...
        for (Produit p : gestionProduit.getListeDesProduits()) supermarche.ajouterProduit(p);
        for (Employe e : gestionEmploye.getListeEmployes()) supermarche.ajouterEmploye(e);
        supermarche.ajouterClient(client1); supermarche.ajouterClient(client2); supermarche.ajouterClient(client3);
        for (Rayon r : gestionRayon.getListeRayons()) supermarche.ajouterRayon(r);
        for (Vente v : gestionCaisseVente.getListeVentes()) supermarche.enregistrerVente(v);
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            Accueil accueil = new Accueil(guFinal,
                    gestionProduit, gestionEmploye, gestionClient,
                    gestionRayon, gestionCaisseVente, gestionFacture);
