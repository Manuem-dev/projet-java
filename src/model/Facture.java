...
public class Facture {
    ...
    // Générer la facture (auto-création à partir des données existantes)
    ...
    public String getContenuFacture() {
        ...
        sb.append("║ Caissier    : ").append(
                String.format("%-28s", vente.getCaissier().getPrenom() + " " + vente.getCaissier().getNom()))
                .append("║\n");
        sb.append("╠══════════════════════════════════════════╣\n");
        ...
        for (Map.Entry<Produit, Integer> entry : occurrences.entrySet()) {
            String ligne = "║ " + entry.getKey().getDesignation()
                    + " x" + entry.getValue()
                    + " = " + String.format("%.2f", entry.getKey().getPrixVente() * entry.getValue()) + " €";
            sb.append(String.format("%-44s", ligne)).append("║\n");
        }
        sb.append("╠══════════════════════════════════════════╣\n");
        sb.append("║ TOTAL         : ").append(String.format("%-26s", String.format("%.2f", vente.calculerTotal()) + " €")).append("║\n");
        sb.append("║ Mode paiement : ").append(String.format("%-26s", paiement.getModePaiement())).append("║\n");
        sb.append("║ Monnaie rendue: ").append(String.format("%-26s", String.format("%.2f", paiement.calculerMonnaieRendue()) + " €")).append("║\n");
        sb.append("╚══════════════════════════════════════════╝\n");
        return sb.toString();
    }
    ...
    public String toString() {
        return "Facture n°" + numeroFacture + " [" + dateFacture + " - " + String.format("%.2f", vente.calculerTotal()) + " €]";
    }
