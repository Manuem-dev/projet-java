...
public class GestionProduit {
	...
	public void ajoutProduitElectronique(int pReference, int pQuantiteStock, double pPrixAchat, double pPrixVente,
			String pDesignation, String pMarque, int pGarantie) {
		if (rechercherProduit(pReference) != null) {
			System.out.println("Référence déjà existante : " + pReference);
			return;
		}
		if (pQuantiteStock <= 0 || pPrixAchat <= 0 || pPrixVente <= 0 || pGarantie < 0 || pDesignation.trim().isEmpty()
				|| pMarque.trim().isEmpty()) {
			System.out.println("Erreur : données invalides pour le produit électronique.");
			return;
		}
		listeDesProduits.add(new ProduitElectronique(pReference, pQuantiteStock, pPrixAchat, pPrixVente, pDesignation,
				pMarque, pGarantie));
		System.out.println("Produit électronique ajouté : " + pDesignation);
	}

	public void ajoutProduitFrais(int pReference, int pQuantiteStock, double pPrixAchat, double pPrixVente,
		...
		if (pQuantiteStock <= 0 || pPrixAchat <= 0 || pPrixVente <= 0 || pDesignation.trim().isEmpty()
				|| pDatePeremption.isBefore(LocalDate.now())) {
			System.out.println("Erreur : données invalides ou date de péremption dépassée.");
			return;
		}
		listeDesProduits.add(new ProduitFrais(pReference, pQuantiteStock, pPrixAchat, pPrixVente, pDesignation,
				pTemperatureConservation, pDatePeremption));
		System.out.println("Produit frais ajouté : " + pDesignation);
	}

	// Modification de produits

	public void modifierProduit(int pReference, int pQuantiteStock, double pPrixAchat, double pPrixVente,
			String pDesignation) {
	...
	// Vérification pour le seuil et la date de péremption
	...
	public ArrayList<ProduitFrais> getProduitsPerimes() {
		ArrayList<ProduitFrais> produitsPerimes = new ArrayList<>();
		for (Produit produit : listeDesProduits) {
			if (produit instanceof ProduitFrais && ((ProduitFrais) produit).estPerime()) {
				produitsPerimes.add((ProduitFrais) produit);
			}
		}
